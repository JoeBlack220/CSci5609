...
void draw()
{
 ...
 textSize(18);
 text("Killdeer", 1500, 800);
 img = loadImage("image/41.jpg");
 image(img, 1300, 810);
 textSize(15);
 description = "Charadrius vociferus (Killdeer) is a species of bird in the family Charadriidae. It is found in the Neotropics and the Nearctic. It is a carnivore.";
 text(description, 1500, 810, 200, 200);
 // draw food chains
...
public void drawFoodChains(int index){
  ...
  String type = allSpecies.get(index).getType();
  ...
  textAlign(CENTER);
  textSize(9);
  text(allSpecies.get(index).getName(), foodChainX, foodChainY + 15);
  textSize(18);
  text("Food chains around " + allSpecies.get(index).getName() +".", foodChainX, foodChainY - 250);
  textSize(12);
  for(List<float []> curSpecies: preyCor){
    for(float[] temp: curSpecies){
